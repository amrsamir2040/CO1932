...
void process(request *pa,request *pb,request *pc,request *pd){
    ...
    {  if(!strcmp("Critical",pro.arr[j].priority))
       ...
       }else{ pd->top=pd->top+1;
         pd->arr[pd->top]=pro.arr[j];
       }}
}
void Join(request *pa,request *pb)
{
    int x,i;
    if(((20)-pb->top)>=(pa->top))
    {
        for(i=pb->top+1,x=0;i<20;i++)
        {
            if(x<pa->top+1)
            {
            pb->arr[i]=pa->arr[x];
...
if(size<s){
            ...
            switch(op)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            printf("lists are Critical,High,Medium and Low");
            printf("Enter frist list name:\n");
            scanf("%s",nameP);
            printf("Enter Second list name:\n");
            scanf("%s",SearchP);
            process(&a,&b,&c,&d);
            Join(&a,&b);
            break;
